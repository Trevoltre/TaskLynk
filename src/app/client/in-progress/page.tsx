  ...
  const fetchJobs = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        const inProgress = data.filter((j: Job) => 
          ['approved', 'assigned', 'in_progress', 'editing'].includes(j.status)
        );
        setJobs(inProgress);
      }
    } catch (error) {
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <div className="container mx-auto px-4 py-8 max-w-7xl">
        <div className="mb-8">
          <h1 className="text-3xl font-bold mb-2">In Progress Orders</h1>
          <p className="text-muted-foreground">
            Active jobs currently being worked on by writers
          </p>
        </div>
          ...
          <div className="space-y-3">
            {jobs.map((job) => (
              <Link key={job.id} href={`/client/jobs/${job.id}`}>
                <Card className="hover:shadow-lg transition-all cursor-pointer border-2 hover:border-blue-400/40 bg-blue-50/30 dark:bg-blue-950/10">
                  <CardHeader>
